...
/* -----------------------------------------------------------------------------------------
   ...
   ---------------------------------------------------------------------------------------*/
include 'platron/PG_Signature.php';
...
// class constructor
    function platron() {
		...
		$this->code = 'platron';
		$this->title = MODULE_PAYMENT_PLATRON_TEXT_TITLE;
		$this->icon = DIR_WS_ICONS . 'paybox.png';
		$this->description = MODULE_PAYMENT_PLATRON_TEXT_DESCRIPTION;
		$this->sort_order = MODULE_PAYMENT_PLATRON_SORT_ORDER;
		$this->enabled = ((MODULE_PAYMENT_PLATRON_STATUS == 'True') ? true : false);

        $this->form_action_url = 'https://paybox.kz/payment.php';
    }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_PLATRON_ZONE > 0) ) {
        $check_flag = false;
        $check_query = vam_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PLATRON_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      ...
      if (isset($_SESSION['cart_platron_id'])) {
        $order_id = substr($_SESSION['cart_platron_id'], strpos($_SESSION['cart_platron_id'], '-')+1);
        ...
        if (vam_db_num_rows($check_query) < 1) {
          ...
          unset($_SESSION['cart_platron_id']);
        }
      }
    ...
    function confirmation() {
      ...
      if (isset($_SESSION['cartID'])) {
        ...
        if (isset($_SESSION['cart_platron_id'])) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($_SESSION['cart_platron_id'], 0, strlen($cartID))) ) {
            $check_query = vam_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN				
          ...
          $_SESSION['cart_platron_id'] = $cartID . '-' . $insert_id;
        }
      }

      return array('title' => MODULE_PAYMENT_PLATRON_TEXT_DESCRIPTION);
    }

    function process_button() {
	  global $insert_id, $languages_id, $order;

		$strCurrency = $order->info['currency'];
		if($strCurrency == 'RUR')
			$strCurrency = 'RUB';
		
		$strDescription = '';
		foreach($order->products as $objProduct){
			$strDescription .= $objProduct['name'];
			if($objProduct['qty'] > 1)
				$strDescription .= "*".$objProduct['qty'];
			$strDescription .= "; ";
		}
		
		$lang_query = vam_db_query("select code from " . TABLE_LANGUAGES . " where languages_id = '" . (int)$languages_id . "'");
		...
		switch (strtolower($lang['code'])) {
			...
			case 'ru':
				...
				break;
			default:
				$language_code = strtolower(DEFAULT_LANGUAGE);
		}
		
		$arrFields = array(
			'pg_merchant_id'		=> MODULE_PAYMENT_PLATRON_MERCHANT_ID,
			'pg_order_id'			=> substr($_SESSION['cart_platron_id'], strpos($_SESSION['cart_platron_id'], '-')+1),
			'pg_currency'			=> $strCurrency,
			'pg_amount'				=> number_format($order->info['total'], 2, '.', ''),
			...
			'pg_success_url'		=> vam_href_link(DIR_WS_CATALOG . FILENAME_CHECKOUT_PROCESS),
			'pg_failure_url'		=> vam_href_link(DIR_WS_CATALOG . FILENAME_CHECKOUT_PAYMENT).'?error_message=' . MODULE_PAYMENT_PLATRON_TEXT_ERROR_MESSAGE,
		...
		if(!empty($order->customer['telephone'])){
			preg_match_all("/\d/", $order->customer['telephone'], $array);
		...
		if(!empty($order->customer['email_address'])){
			$arrFields['pg_user_email'] = $order->customer['email_address'];
			$arrFields['pg_user_contact_email'] = $order->customer['email_address'];
		}
		$arrFields['pg_sig'] = PG_Signature::make('payment.php', $arrFields, MODULE_PAYMENT_PLATRON_SECRET_KEY);
		
		$process_button_string = '';
		foreach($arrFields as $strName => $strValue){
			$process_button_string .= "<input type='hidden' value='$strValue' name='$strName'>";
		}
...
// unregister session variables used during checkout
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = vam_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PLATRON_STATUS'");
    ...
    function install() {
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PLATRON_STATUS', 'True', '6', '1', 'vam_cfg_select_option(array(\'True\', \'False\'), ', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) values ('MODULE_PAYMENT_PLATRON_ZONE', '0', '6', '8', 'vam_get_zone_class_title', 'vam_cfg_pull_down_zone_classes(', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PLATRON_ALLOWED', '', '6', '2', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PLATRON_MERCHANT_ID', '', '6', '3', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PLATRON_SECRET_KEY', '', '6', '4', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PLATRON_LIFETIME', '', '6', '5', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) values ('MODULE_PAYMENT_PLATRON_SORT_ORDER', '0', '6', '6', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PLATRON_ORDER_STATUS_OK_ID', '0', '6', '8', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PLATRON_ORDER_STATUS_FAILED_ID', '0', '6', '8', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) values ('MODULE_PAYMENT_PLATRON_ORDER_STATUS_PENDING_ID', '0', '6', '8', 'vam_cfg_pull_down_order_statuses(', 'vam_get_order_status_name', now())");
		vam_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PAYMENT_PLATRON_TEST_MODE', 'test', '6', '9', 'vam_cfg_select_option(array(\'test\', \'production\'), ', now())");
	}
	...
	function keys() {
		return array('MODULE_PAYMENT_PLATRON_STATUS', 'MODULE_PAYMENT_PLATRON_ZONE', 'MODULE_PAYMENT_PLATRON_ALLOWED', 'MODULE_PAYMENT_PLATRON_MERCHANT_ID', 'MODULE_PAYMENT_PLATRON_SECRET_KEY', 'MODULE_PAYMENT_PLATRON_LIFETIME', 'MODULE_PAYMENT_PLATRON_SORT_ORDER',  'MODULE_PAYMENT_PLATRON_SORT_ORDER', 'MODULE_PAYMENT_PLATRON_ORDER_STATUS_OK_ID', 'MODULE_PAYMENT_PLATRON_ORDER_STATUS_FAILED_ID', 'MODULE_PAYMENT_PLATRON_ORDER_STATUS_PENDING_ID', 'MODULE_PAYMENT_PLATRON_TEST_MODE');
	}
